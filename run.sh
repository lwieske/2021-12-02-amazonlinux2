...
ssh -o StrictHostKeyChecking=no -i ${KEY_NAME}.pem ec2-user@${INSTANCE_PUBLIC_IP} -tt <<EOF
    set -x
    sudo yum -y install https://packages.microsoft.com/config/centos/7/packages-microsoft-prod.rpm
    sudo yum -y install dotnet-sdk-6.0
    source /etc/profile.d/dotnet-cli-tools-bin-path.sh
    dotnet --info
    sleep 20
    sudo yum -y remove packages-microsoft-prod dotnet-sdk-6.0
    exit
EOF
